#!/usr/bin/env Rscript
# Script for generating methrix, bsseq, RnBeads, and methylKit objects
...
parser$add_argument("-o", default="./objects/",
...
#---------------------------------------------------------------------------------------------------------
import_bsseq = function(anno, h5 = FALSE){
  ...
  bsseq_obj <- bsseq::read.bismark(anno$bedgraph_files,
                                   loci = cpg_loci,
                                   colData = anno,
                                   rmZeroCov = FALSE,
                                   strandCollapse = TRUE,
...
import_methrix = function(anno, h5 = FALSE){
  ...
  meth_obj = methrix::read_bedgraphs(
    ...
    zero_based = FALSE,
    stranded = TRUE,
    collapse_strands = TRUE, coldata = sample_anno, h5 = backend, h5_dir = h5dir)
...
run_methykit_import = function(anno){
  methylKit_obj_raw = methylKit::methRead(
    location = as.list(anno$bedgraph_files),
    sample.id = as.list(anno$Sample_Names),
    assembly = "hg38",
    pipeline = "bismarkCoverage",
    treatment = anno$treatment)
  methylKit_obj <- methylKit::unite(methylKit_obj_raw, destrand=FALSE, min.per.group = 1L)
  list(raw = methylKit_obj_raw, united = methylKit_obj)
}

import_methylkit = function(anno){
  suppressMessages(suppressPackageStartupMessages((library("methylKit"))))
  suppressMessages(suppressPackageStartupMessages((library("microbenchmark"))))
  mobj <- run_methykit_import(anno = anno)
  mobj
}

#---------------------------------------------------------------------------------------------------------
run_rnbeads = function(anno, dir = "./"){
  ...
  RnBeads::rnb.options(import.bed.style = "bismarkCov",
                       region.types = c(),
                       disk.dump.big.matrices = FALSE, assembly = "hg38")
...
if(tool == "methrix"){
  res = import_methrix(anno = sample_anno)
  saveRDS(object = res, file = paste0(op_dir,"/", tool, "_", nfiles, "samples.RDs"), version = 2)
}else if(tool == "bsseq"){
  res = import_bsseq(anno = sample_anno)
  saveRDS(object = res, file = paste0(op_dir,"/", tool, "_", nfiles, "samples.RDs"), version = 2)
}else if(tool == "methylKit"){
  res = import_methylkit(anno = sample_anno)
  saveRDS(object = res[[1]], file = paste0(op_dir,"/", tool, "Raw_", nfiles, "samples.RDs"), version = 2)
  saveRDS(object = res[[2]], file = paste0(op_dir,"/", tool, "_", nfiles, "samples.RDs"), version = 2)
}else if(tool == "RnBeads"){
    res = run_rnbeads(anno = sample_anno, dir = cov_dir)
    saveRDS(object = res, file = paste0(op_dir,"/", tool, "_", nfiles, "samples.RDs"), version = 2)
}
